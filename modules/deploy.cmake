...
function(_CDU_collect_runtime_dirs out_var main_target)
    ...
    foreach(_t IN LISTS _to_scan)
        ...
        foreach(_lk IN LISTS _links)
            if(TARGET "${_lk}")
                _CDU_get_real_target_name(_lk_real "${_lk}")
                if(NOT TARGET "${_lk_real}")
                    continue()
                endif()

                get_target_property(_lk_type "${_lk_real}" TYPE)

                if(_lk_type STREQUAL "INTERFACE_LIBRARY")
                    get_target_property(_iface_links "${_lk_real}" INTERFACE_LINK_LIBRARIES)
                    foreach(_il IN LISTS _iface_links)
                        if(TARGET "${_il}")
                            _CDU_get_real_target_name(_il_real "${_il}")
                            if(TARGET "${_il_real}")
                                get_target_property(_il_type "${_il_real}" TYPE)
                                if(NOT _il_type STREQUAL "INTERFACE_LIBRARY")
                                    list(APPEND _dirs "$<TARGET_FILE_DIR:${_il_real}>")
                                endif()
                            endif()
                        endif()
                    endforeach()
                else()
                    get_target_property(_loc "${_lk_real}" IMPORTED_LOCATION_${CMAKE_INSTALL_CONFIG_NAME})
                    if(NOT _loc)
                        get_target_property(_loc "${_lk_real}" IMPORTED_LOCATION)
                    endif()
                    if(_loc)
                        ...
                        list(APPEND _dirs "$<TARGET_FILE_DIR:${_lk_real}>")
                    endif()
                endif()
            else()
                if(EXISTS "${_lk}")
    ...
    if(app_packages)
        foreach(package_name IN LISTS app_packages)
            if(NOT TARGET ${package_name})
                ...
                set(real_package_name ${package_name})
            endif()
